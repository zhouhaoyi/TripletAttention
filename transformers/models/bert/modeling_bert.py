...
class BertSelfAttention(nn.Module):
    def __init__(self, config, layer):
        ...
        # parameter of triplet attention
        self.group_size = config.group_size # number of triplet attention head
        self.agg_type = config.agg_type # aggregation type when using L*L cross product
        self.cross_type = config.cross_type # cross product type (0: L cross product with permutation, 1: L*L cross product)
        self.absolute_flag = config.absolute_flag # whether to use absolute value of triplet attention
        self.random_flag = config.random_flag # permutation type (0: multi permutation, 1:only random permutation)
        self.permute_type = [int(l) for l in config.permute_type.replace(' ','').split(',')] # permutaion type groups
        self.permute_back = config.permute_back # whether to do permutation inverse
        self.layer = layer
        self.Tlayers = [int(l) for l in config.Tlayers.replace(' ','').split(',')] # layers which use triplet attention heads

        self.key2_flag = config.key2_flag # whether to use key2 linear layer to get key_triplet2
        if self.key2_flag==0 and self.layer in self.Tlayers:
            self.key2 = nn.Linear(config.hidden_size, self.attention_head_size*config.group_size*3)

        self.head_choice = config.head_choice
        if self.head_choice!=0: # whether to choose triplet attention heads randomly
            import numpy as np
            np.random.seed(self.head_choice)
            self.heads_triplet = np.random.choice(np.arange(0,self.num_attention_heads), self.group_size*3, replace=False)
            self.heads_triplet.sort()
            self.heads_normal = np.setdiff1d(np.arange(0,self.num_attention_heads), self.heads_triplet)
    ...
    def forward(
        ...
        if self.layer in self.Tlayers: # if this layer need to set triplet attention
            # get key_triplet and query_triplet
            if self.head_choice==0: # choose the last 3*group_size heads as triplet attention heads
            ...
            else: # randomly choose 3*group_size heads as triplet attention heads
            ...
            # increase the number of multi-heads and set dimention of each head as 3
            ...
            # cross product
            if cross_type==0: # cross: L and random L (need to do permutation)
                # get key_triplet2
                if key2_flag==0: # use key2 linear layer
                    key_triplet2 = self.key2(hidden_states) # [B, L, D*H]
                ...
                elif random_flag==1: # random permute
                    import numpy as np
            ...
            # get triplet attention score
            # Q [B, newH(64), L, D(3*gs)]; 
            attn_triplet = torch.matmul(query_triplet, K_cross.transpose(-1, -2)) # [B, H(64), L, L]

            # permutation inverse
            if cross_type==0 and permute_back==0 and random_flag==0:
                permutes = len(permute_type)
        ...
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2)) # [B, H(12-3*gs), L, L]
        ...
        if self.layer in self.Tlayers:
            # apply triplet attention on selected value
            attn_triplet = attn_triplet / math.sqrt(self.attention_head_size)
        ...
        context_layer = torch.matmul(attention_probs, value_layer) # [B, H(12-3*gs), L, L] or [B, H(12), L, D(64)]
        context_layer = context_layer.permute(0, 2, 1, 3).contiguous() # [B, L, H(12-3*gs), D(64)] or [B, L, H(12), D(64)]
        B, L, H, D = context_layer.shape
        
        # heads random choice
        if self.layer in self.Tlayers:
            if self.head_choice!=0: # randomly choose 3*group_size heads as triplet attention heads
        ...
        if self.layer in self.Tlayers:
            if self.head_choice==0: # choose the last 3*group_size heads as triplet attention heads
                context_layer = torch.cat([context_layer, context_triplet], dim=-1)

        # new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,) # [B, L, 12*64 {H(12)*D(64)}]
        # context_layer = context_layer.view(*new_context_layer_shape)

        if self.layer in self.Tlayers:
            outputs = (context_layer, [attention_probs, attn_triplet]) if output_attentions else (context_layer,)
        else:
            outputs = (context_layer, [attention_probs, None]) if output_attentions else (context_layer,)
